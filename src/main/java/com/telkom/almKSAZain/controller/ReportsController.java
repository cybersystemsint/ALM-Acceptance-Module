...
public class ReportsController {
    ...
    public Map<String, Object> acceptanceReport(@RequestBody String req) {
        ...
        //  String sql = "SELECT * FROM `acceptanceReport` WHERE 1=1";
        ...
        String countSql = "SELECT COUNT(*) FROM acceptanceReport" + whereClause;
        ...
        String sql = "SELECT * FROM `acceptanceReport` "
...
//        int totalRecords = jdbcTemplate.queryForObject(countSql, Integer.class);
        String countSql = "SELECT COUNT(*) FROM tb_ItemCodeSubstitute" + whereClause;
        int totalRecords = jdbcTemplate.queryForObject(countSql, Integer.class, params.toArray());
...
//        return result;
    ...
    public Map<String, Object> getAllPurchaseOrders(@RequestBody String req) {
        ...
        //String searchQuery = obj.get("searchQuery").getAsString();
        ...
        // Check if columnName and searchQuery are provided
        if (!columnName.isEmpty() && !searchQuery.isEmpty()) {
...
//        List<Map<String, Object>> result = jdbcTemplate.queryForList(finalSql);
    ...
    public Map<String, Object> getNestedPurchaseOrders(@RequestBody String req) {
        ...
        if (!supplierId.equalsIgnoreCase("0") && !poID.equalsIgnoreCase("0")) {
            ...
            params2.add(poID);
            // whereClause += " AND PO.vendorNumber='" + supplierId + "' AND PO.poNumber='" + poID + "'";
        } else if (!supplierId.equalsIgnoreCase("0") && poID.equalsIgnoreCase("0")) {
            whereClause += " AND PO.vendorNumber = ?";
            params2.add(supplierId);
            //  whereClause += " AND PO.vendorNumber='" + supplierId + "'";
        } else if (supplierId.equalsIgnoreCase("0") && !poID.equalsIgnoreCase("0")) {
            whereClause += " AND PO.poNumber = ?";
            params2.add(poID);
            //whereClause += " AND PO.poNumber='" + poID + "'";
        }
        ...
        // Step 2: If pagination is needed, fetch line items for the unique POs with pagination
//        String paginatedLineItemsSql = "SELECT * FROM tb_PurchaseOrder PO WHERE PO.poNumber IN (" + String.join(",", uniquePONumbers.stream().map(po -> "'" + po + "'").collect(Collectors.toList())) + ") LIMIT " + size + " OFFSET " + (page - 1) * size;
//        List<Map<String, Object>> paginatedLineItems = jdbcTemplate.queryForList(paginatedLineItemsSql);
        // Step 3: Group line items by PO number
