...
// Removes finished/failed export jobs (file + job-record) once they're old enough that
// nobody is still going to download them, and resolves jobs that got stuck RUNNING/PENDING
// (e.g. because the app was redeployed mid-export) so they don't sit reporting "in progress"
// forever. Mirrors ALM-Site-Management's ExportJobCleanupService (same file-sidecar job
// store, not a DB table - see ExportJob's javadoc). Covers every job-based export endpoint
// in this controller (combined-view, combined-view/export, export-combined-view-approvers),
// not just the one being added alongside this class.
//
// No distributed lock here: deleting an already-deleted job file is a no-op, and file
// deletion is per-instance local disk, so a second instance running this at the same time
// is harmless - there's no non-idempotent side effect like sending an email to protect
// against.
@Service
public class ExportJobCleanupService {

    private static final Logger logger = LogManager.getLogger(ExportJobCleanupService.class);
