 ...
 *
 * NOTE: findByPoIdIn and findByRecordNoIn are used by QuantityContext pre-loading
 * in DccPOOptimizedServiceImpl to eliminate N+1 queries during export.
 */
@Repository
public interface TbDccLnRepository extends JpaRepository<DCCLineItem, Long> {
    ...
    List<DCCLineItem> findByUplLineNumberAndLineNumberAndPoId(
            String uplLineNumber, String lineNumber, String poId);

    List<DCCLineItem> findByPoIdAndLineNumberAndUplLineNumber(
            String poId, String lineNumber, String uplLineNumber);

    boolean existsByPoIdAndLineNumberAndUplLineNumber(
            String poId, String lineNumber, String uplLineNumber);
