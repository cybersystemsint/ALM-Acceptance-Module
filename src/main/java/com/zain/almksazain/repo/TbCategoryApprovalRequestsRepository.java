...
public interface TbCategoryApprovalRequestsRepository extends JpaRepository<TbCategoryApprovalRequests, Long> {
    ...
    // Batch fetch all approvals for a list of request record nos
    @Query("""
        SELECT a FROM TbCategoryApprovals a
        WHERE a.approvalRecordId IN :requestIds
        """)
    List<TbCategoryApprovals> findByApprovalRecordIdIn(@Param("requestIds") List<Long> requestIds);
}