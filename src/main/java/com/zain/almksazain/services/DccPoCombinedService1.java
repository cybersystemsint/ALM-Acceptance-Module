...
@Service
public class DccPoCombinedService1 {
    ...
    @Autowired
    private tbDepartmentRepo tbDepartmentRepo;
    public Map<String, Object> getAgingReport(String supplierId, String columnName, String searchQuery, int page, int size) {
        ...
        dccColumns.put("vendorName", Map.entry("vendorName", "string"));
        ...
        for (DCC dcc : dccList) {
            ...
            groupedRow.put("vendorName", dcc.getVendorName());
            ...
            // groupedRow.put("requestAmountSAR)", requestAmountSAR);
           
            Double requestAmountSAR = 0.0;
            try {
                requestAmountSAR = calculateRequestAmount(ln, uplAcptRequestValue);
            } catch (Exception e) {
                e.printStackTrace();
                requestAmountSAR = 0.0;
            }
            groupedRow.put("requestAmountSAR", requestAmountSAR);
            // Double requestAmountSAR = calculateRequestAmount(ln, uplAcptRequestValue);
            // groupedRow.put("requestAmountSAR", requestAmountSAR);
            groupedRow.put("poId", po != null ? po.getPoNumber() : dcc.getPoNumber());
            // groupedRow.put("createdBy", dcc.getCreatedBy());
            // groupedRow.put("requestedBy", dcc.getCreatedBy());
            // groupedRow.put("createdByName", dcc.getCreatedBy());
            String createdByUsername = dcc.getCreatedBy();
            String createdByFullName = null;
            if (createdByUsername != null) {
                Optional<UserAccount> createdByUserOpt = userAccountRepo.findByUsername(createdByUsername);
                if (createdByUserOpt.isPresent()) {
                    createdByFullName = createdByUserOpt.get().getFullName();
                }
            }
            System.out.println("createdByFullName: " + createdByFullName);
            groupedRow.put("createdBy", createdByFullName);
            groupedRow.put("requestedBy", createdByFullName);
            groupedRow.put("createdByName", createdByFullName);
            ...
            if (!approvals.isEmpty()) {
             ...
             String pendingApproverName = pendingApprovers;
          
              if (pendingApproverName != null) {
                  Optional<UserAccount> userOpt = userAccountRepo.findByUsername(pendingApproverName);
                  if (userOpt.isPresent()) {
                      UserAccount pendingUser = userOpt.get();
                      tb_department dep = pendingUser.getDepartment();
                      if (dep != null) {
                          pendingApproverDepartmentName = dep.getDepartmentName();
                      }
                      pendingApproverFullName = pendingUser.getFullName();
                  }
              }  
             if (pendingApproverName != null) {
                //  Optional<UserAccount> userOpt = userAccountRepo.findByFullName(pendingApproverName);
                 Optional<UserAccount> userOpt = userAccountRepo.findByUsername(pendingApproverName);
                 if (userOpt.isPresent()) {
                     UserAccount pendingUser = userOpt.get();
                     tb_department dep = pendingUser.getDepartment();
           ...
           System.out.println("pendingApproverDepartmentName: " + pendingApproverDepartmentName);
            ...
            groupedRow.put("pendingApprovers", pendingApproverFullName);
            groupedRow.put("departmentName", pendingApproverDepartmentName);
...
private Double calculateRequestAmount(DCCLineItem lineItem, Double deliveredQty) {
    if (lineItem == null || deliveredQty == null) return 0.0;
    String poId = lineItem.getPoId();
    String lineNumberStr = lineItem.getLineNumber();
    if (poId == null || lineNumberStr == null || lineNumberStr.trim().isEmpty()) {
        return 0.0;
    }
    Integer lineNumber;
    try {
    ...
    PurchaseOrderTb po = purchaseOrderRepository.findByPoNumberAndLineNumber(poId, lineNumber);
    Double unitPriceInSAR = (po != null) ? po.getUnitPriceInSAR() : 0.0;
    double unitPrice = (unitPriceInSAR != null) ? unitPriceInSAR : 0.0;
    double qty = (deliveredQty != null) ? deliveredQty : 0.0;
    ...
    // Fetch all line items for this DCC record
    List<DCCLineItem> lineItems = dccLineRepo.findByDccIdAndDccStatusNotIn(
        String.valueOf(dccRecordNo),
        Arrays.asList("incomplete", "rejected")
    );
    // Sum their deliveredQty
