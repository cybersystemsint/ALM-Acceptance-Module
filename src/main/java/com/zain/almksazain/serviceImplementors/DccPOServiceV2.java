...
public class DccPOServiceV2 {
    ...
    private DccPOFetchResultV2 fetchDccPOCombinedView(String supplierId, String pendingApprovers, int page, int size, String columnName, String searchQuery, boolean exporting, String operator) {
        try {
            if (page < 1 || size < 1) {
            ...
            if (dccList.isEmpty()) {
                ...
                return new DccPOFetchResultV2(new ArrayList<>(), totalFilteredRecords, totalUnfilteredRecords);
            }
...
//            boolean fetchParentOnly = !(columnName != null && columnName.equalsIgnoreCase("recordNo") && searchQuery != null && !searchQuery.isEmpty());
                ...
                result = dccList.parallelStream()
                        ...
                        .flatMap(dcc -> {
                            ...
                            if (dccLnList.isEmpty()) {
                                logger.warn("No DCC_LN records found for DCC ID: {}. Skipping.", dcc.getRecordNo());
            ...
            return new DccPOFetchResultV2(result, totalFilteredRecords, totalUnfilteredRecords);
        } catch (Exception ex) {
        ...
        for (DCCLineItem dccLn : dccLnList) {
            ...
            if (uplList != null) {
                for (tb_PurchaseOrderUPL upl : uplList) {
                    ...
                    if (condition) matchedUpls.add(upl);
                }
            }
            ...
            if (matchedUpls.isEmpty()) {
                ...
                populatePurchaseOrderAndUplFields(dto, dccLn, purchaseOrder, null);
                calculateQuantitiesAndApprovals(dto, dcc, purchaseOrder, null, dccLnByUplLineNumber, latestApprovalRequest);
                dtos.add(dto);
                continue;
            }
...
//        }
    ...
    public static class DccPOFetchResultV2 {
        private final List<DccPOCombinedViewDTO> data;
        private final Long totalRecordsInDb;
        private final Long totalFilteredRecords;

        public DccPOFetchResultV2(List<DccPOCombinedViewDTO> data, Long totalFilteredRecords, Long totalRecordsInDb) {
            this.data = data;
