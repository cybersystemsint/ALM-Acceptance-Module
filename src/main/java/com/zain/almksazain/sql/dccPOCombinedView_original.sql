-- Snapshot of ALM_ZAIN_KSA.dccPOCombinedView's ORIGINAL definition, before the fix applied
-- earlier in this engagement (root-caused to a JOIN fan-out causing duplicate line items on
-- the acceptance-request edit flow; fixed directly in the DB, predating this snapshot).
-- Source: extracted from the full mysqldump /Volumes/Jeff Drive/ALM_ZAIN_KSA_2026_09_21.sql
-- (dump file timestamp: 2026-09-21 00:02, i.e. captured before this session's work that day).
-- NOT from a live `SHOW CREATE VIEW` - the local MySQL instance's own `dccpocombinedview` is
-- only a placeholder stub (`SELECT 1 AS col, 1 AS col, ...`, ~1.2KB), not the real view logic,
-- so the dump was the only available source for the original state.
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `dccPOCombinedView` AS with `LatestApprovalRequests` as (select `tb_Category_Approval_Requests`.`recordNo` AS `recordNo`,`tb_Category_Approval_Requests`.`recordDateTime` AS `recordDateTime`,`tb_Category_Approval_Requests`.`acceptanceRequestRecordNo` AS `acceptanceRequestRecordNo`,`tb_Category_Approval_Requests`.`poNumber` AS `poNumber`,`tb_Category_Approval_Requests`.`tableName` AS `tableName`,`tb_Category_Approval_Requests`.`poLineItemDescription` AS `poLineItemDescription`,`tb_Category_Approval_Requests`.`vendorName` AS `vendorName`,`tb_Category_Approval_Requests`.`requestedBy` AS `requestedBy`,`tb_Category_Approval_Requests`.`createdBy` AS `createdBy`,`tb_Category_Approval_Requests`.`itemCategoryCode` AS `itemCategoryCode`,`tb_Category_Approval_Requests`.`scope` AS `scope`,`tb_Category_Approval_Requests`.`status` AS `status`,`tb_Category_Approval_Requests`.`approvedDate` AS `approvedDate`,row_number() OVER (PARTITION BY `tb_Category_Approval_Requests`.`acceptanceRequestRecordNo` ORDER BY `tb_Category_Approval_Requests`.`recordDateTime` desc )  AS `rn` from `tb_Category_Approval_Requests`) select distinct `DCC`.`recordNo` AS `dccRecordNo`,
 ...
 (select coalesce(sum(`subquery`.`POAcceptanceQty`),0) from (select `upl`.`poNumber` AS `poNumber`,`upl`.`poLineNumber` AS `poLineNumber`,((`upl`.`uplLineQuantity` * `upl`.`poLineQuantity`) / nullif((`upl`.`poLineQuantity` * `upl`.`poLineUnitPrice`),0)) AS `POAcceptanceQty` from `tb_PurchaseOrderUPL` `upl` where (`upl`.`uplLineQuantity` > 0)) `subquery` where ((`subquery`.`poNumber` = `upl`.`poNumber`) and (`subquery`.`poLineNumber` = `upl`.`poLineNumber`))) AS `POLineAcceptanceQty`,
 (select sum(`subquery`.`POLineAcceptanceQty`) AS `poPendingQuantity` from (select `upl`.`poNumber` AS `poNumber`,`upl`.`poLineNumber` AS `poLineNumber`,((`upl`.`uplLineQuantity` * `upl`.`poLineQuantity`) / nullif((`upl`.`poLineQuantity` * `upl`.`poLineUnitPrice`),0)) AS `POLineAcceptanceQty` from `tb_PurchaseOrderUPL` `upl` where (`upl`.`uplLineQuantity` > 0)) `subquery` where ((`subquery`.`poNumber` = `upl`.`poNumber`) and (`subquery`.`poLineNumber` = `upl`.`poLineNumber`))) AS `poPendingQuantity`,
...
from ((((`tb_DCC` `DCC` join `tb_PurchaseOrder` `HD` on((`DCC`.`poNumber` = `HD`.`poNumber`))) left join `LatestApprovalRequests` `AR` on(((`DCC`.`recordNo` = `AR`.`acceptanceRequestRecordNo`) and (`AR`.`rn` = 1)))) join `tb_DCC_LN` `LN2` on((`DCC`.`recordNo` = `LN2`.`dccId`))) left join `tb_PurchaseOrderUPL` `upl` on((`DCC`.`poNumber` = `upl`.`poNumber`))) where (0 <> (case when (length(`LN2`.`uplLineNumber`) > 0) then ((`LN2`.`uplLineNumber` = `upl`.`uplLine`) and (`upl`.`poLineNumber` = `LN2`.`lineNumber`) and (`upl`.`poNumber` = `DCC`.`poNumber`)) else ((`HD`.`lineNumber` = `LN2`.`lineNumber`) and (`HD`.`poNumber` = `DCC`.`poNumber`)) end)) 
